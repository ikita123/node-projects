...
async function inser_data() {
    ...
    const insertResult = await collection.insertMany([{ "name": "nikita", "password": "872004" }]);
...
async function update_data() {
    ...
    const db = client.db("MONGO")
    const collection = db.collection('users');
    const updateResult = await collection.updateOne({ "name": "nikita", "password": "872004" }, { $set: { "name": "Tikaram", "password": "872004" } });
...
async function delete_data() {
    ...
    const db = client.db("MONGO")
    const collection = db.collection('users');
    const deleteResult = await collection.deleteMany({ "name": "nikita", "password": "872004" });
